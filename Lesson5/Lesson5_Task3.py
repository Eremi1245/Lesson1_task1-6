...
with open('employees_file.txt') as employees:
    ...
    for string in employees.readlines():
        ...
        average_sum += int(result[0].replace(' ', ''))
        count += 1  # ставлю счетчик потому что невозможно использовать функцию len(employees.readlines())
        ...
        # обыгрываем ошибку если число записано без пробела
            ...
            print(string.split(' ')[0])
    average_sum_employees = round(average_sum / count, 2)
    print(f"Средняя величина дохода работников: {average_sum_employees} рублей")
